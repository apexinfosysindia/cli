...
and VL805 where present). A reboot is required for the new firmware to take effect.`,
	Example: `
  apex os boards raspberrypi firmware update`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if pending, _ := info.Data["update_pending"].(bool); pending {
			fmt.Println("A firmware update is already pending. Reboot the device using `apex host reboot` to finish the installation.")
			return
		}
		...
		if err != nil {
			...
			fmt.Println("\nFirmware update applied. Reboot the device using `apex host reboot` to finish the installation.")
		} else {
			ExitWithError = true
		}
	},
