...
Welcome to the ApexOS command line interface.
`
...
var bannerCmd = &cobra.Command{
	Use:     "banner",
	Aliases: []string{"ba"},
	Short:   "Prints the CLI ApexOS banner along with some useful information",
	Example: `
  apex banner
	`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if !nowait {
			...
			for i := range 180 { // 3 minutes timeout
				...
				if err == nil && netinfo != nil {
					...
					if exist && len(netifaces.([]any)) > 0 {
						fmt.Println("ApexOS Supervisor is running!")
			...
			if !supervisorReady {
				fmt.Println("Supervisor is taking longer than expected to start. Use 'apex supervisor logs' to check logs.")
				return
			}
		}
