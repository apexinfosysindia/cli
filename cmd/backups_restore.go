...
var backupsRestoreCmd = &cobra.Command{
	Use:   "restore [slug]",
	Short: "Restores a ApexOS backup",
	Long: `
When something goes wrong, this command allows you to restore a previously
take ApexOS backup on your system.`,
	Example: `
  apex backups restore c1a07617
  apex backups restore c1a07617 --app core_ssh --app core_mosquitto
  apex backups restore c1a07617 --folders apexos`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		apexos, err := cmd.Flags().GetBool("apexos")
		if err == nil && cmd.Flags().Changed("apexos") {
			options["apexos"] = apexos
...
func init() {
	...
	backupsRestoreCmd.Flags().BoolP("apexos", "", true, "Restore apexos (default true), performs a partial restore when set to false")
	...
	backupsRestoreCmd.RegisterFlagCompletionFunc("apexos", boolCompletions)
