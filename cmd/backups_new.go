...
var backupsNewCmd = &cobra.Command{
	...
	Short:   "Create a new ApexOS backup",
	Long: `
This command can be used to trigger the creation of a new ApexOS
backup.`,
	Example: `
  apex backups new
  apex backups new --app core_ssh --app core_mosquitto
  apex backups new --folders apexos
  apex backups new --uncompressed
`,
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		ExcludeDB, err := cmd.Flags().GetBool("apexos-exclude-database")
		if err == nil && cmd.Flags().Changed("apexos-exclude-database") {
			options["apexos_exclude_database"] = ExcludeDB
		}
...
func init() {
	...
	backupsNewCmd.Flags().Bool("apexos-exclude-database", false, "Exclude the ApexOS database file from backup")
	...
	backupsNewCmd.Flags().Lookup("apexos-exclude-database").NoOptDefVal = "false"
	...
	backupsNewCmd.RegisterFlagCompletionFunc("apexos-exclude-database", boolCompletions)
