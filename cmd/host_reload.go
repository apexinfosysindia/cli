package cmd

import (
	"log/slog"

	helper "github.com/apexinfosysindia/cli/client"
	"github.com/spf13/cobra"
)

var hostReloadCmd = &cobra.Command{
	Use:     "reload",
	Aliases: []string{"update", "refresh", "re"},
	Short:   "Reload information from the host machine",
	Long: `
This commands reload the information ApexOS has on the hostmachine.
If some setting are changed outside of ApexOS, this commands updates
the internals of ApexOS.`,
	Example: `
  apex host reload`,
	ValidArgsFunction: cobra.NoFileCompletions,
	Args:              cobra.NoArgs,
	Run: func(cmd *cobra.Command, args []string) {
		slog.Debug("host reload", "args", args)

		section := "host"
		command := "reload"

		resp, err := helper.GenericJSONPost(section, command, nil)
		if err != nil {
			helper.PrintError(err)
			ExitWithError = true
		} else {
			ExitWithError = !helper.ShowJSONResponse(resp)
		}
	},
}

func init() {
	hostCmd.AddCommand(hostReloadCmd)
}
