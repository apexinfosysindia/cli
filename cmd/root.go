...
import (
	...
	"github.com/apexinfosysindia/cli/spinner"
...
var rootCmd = &cobra.Command{
	...
	Short: "A small CLI program to control ApexOS",
	Long: `
The ApexOS CLI is a small and simple command line utility that allows
you to control and configure different aspects of ApexOS`,
...
func init() {
	...
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "Optional config file (default is $HOME/.apexos.yaml)")
	rootCmd.PersistentFlags().StringVar(&endPoint, "endpoint", "", "Endpoint for ApexOS Supervisor (default is 'supervisor')")
	rootCmd.PersistentFlags().StringVar(&logLevel, "log-level", "", "Log level (defaults to "+slog.LevelWarn.String()+")")
	rootCmd.PersistentFlags().StringVar(&apiToken, "api-token", "", "ApexOS Supervisor API token")
...
func initConfig() {
	...
	if cfgFile != "" {
		...
		// Search config in home directory with name ".apexos" (without extension).
		viper.AddConfigPath(home)
		slog.Debug("Adding homedir to searchpath", "homedir", home)
		viper.SetConfigName(".apexos")
	}
